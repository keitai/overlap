class TherapistsController < ApplicationController
  ...
  def show
    @note = Note.new
    @notes = Note.where(:therapist_id => @therapist).order("created_at DESC")
    @overlaps = Appointment.where(:therapist_id => @therapist).order("created_at DESC")
  end
  ...
  private
    ...
    def therapist_params
      params.require(:therapist).permit(:first_name, :last_name, :email, :phone_number, :location)  
    end
